		...
		return {
			...
			controller: ['$http', function ($http) {
				...
				this.submitForm = function () {
					...
					$http.get("/cost_comparisons/" + project_name_encoded + ".json").success(function (data, status, headers, config) {
						// タイトルを元に戻し、データを保持
						thisController.pageTitle = thisController.pageTitleDefault;
				...
				this.getTermText = function (dataArray) {
					...
					if (dataArray && dataArray.length > 0) {
						...
						quotationTermText = new Date(dataArray[0].updated_at).toLocaleString('ja-JP', options).replace("JST", "") + " 〜 " + new Date(dataArray[dataArray.length - 1].updated_at).toLocaleString('ja-JP', options).replace("JST", "");
					}
				...
				this.getTotalManDay = function (array) {
					...
					return !array ? "" : array.reduce(function (prevValue, currentValue) {
						return prevValue + currentValue.man_day;
					}, 0);
				};
				...
				this.downloadCsv = function () {
					...
					thisController.waiting = true;
					var project_name_encoded = encodeURIComponent(thisController.selectedProject);
					$http.get("/cost_comparisons/download/" + project_name_encoded + ".csv").success(function (data, status, headers, config) {
